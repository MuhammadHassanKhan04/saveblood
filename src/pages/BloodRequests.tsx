...
import { Phone, Mail, MapPin, HeartPulse, Trash2, AlertCircle } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";
import { useGlobalData } from "@/contexts/DataContext";
import { deleteBloodRequest } from "@/utils/storage";
import { toast } from "sonner";

const BloodRequests = () => {
  const { t } = useLanguage();
  const { requests, refreshData } = useGlobalData();
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter((request) => {
      const bloodMatch = bloodFilter === "all" || request.blood_group === bloodFilter;
      const cityMatch = cityFilter === "all" || request.city === cityFilter;
      return bloodMatch && cityMatch;
    });
  }, [requests, bloodFilter, cityFilter]);
  ...
  const handleDeleteConfirm = async () => {
    if (password === "rmbhahnh") {
      const success = await deleteBloodRequest(selectedRequestId);
      if (success) {
      ...
      <Dialog open={!!selectedRequest} onOpenChange={() => setSelectedRequest(null)}>
        <DialogContent className="sm:max-w-[500px] bg-card">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete blood request information and contact details
            </DialogDescription>
          </DialogHeader>
          {selectedRequest && (
            <div className="space-y-4 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">Blood Group Needed</p>
                  <p className="text-lg font-semibold text-foreground">{selectedRequest.blood_group}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">Patient Age</p>
                  <p className="text-lg font-semibold text-foreground">{selectedRequest.age} years</p>
                </div>
              </div>
              ...
              <div className="space-y-3 border-t border-border pt-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-foreground font-medium">{selectedRequest.city}, {selectedRequest.country}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">Reason for Request</p>
                    <p className="text-foreground font-medium">{selectedRequest.reason}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Please enter the admin password to delete this blood request.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
            <Input
